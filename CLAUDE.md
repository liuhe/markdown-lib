...
| `FileTreeWatcher.swift` | `FSEventStream` wrapper. Recursive, debounced (~350 ms). Fires `onChange` on main with the deduped set of changed paths from the batch; the caller is expected to decide whether the batch is worth reacting to (see `WorkspaceStore.shouldRescan`). |
